// src/pages/Dashboard/Dispensers/DispensersPage.tsx
...
export default function DispensersPage() {
  ...
  // React Query hooks
  const { data: dispensers = [], isLoading, error } = useDispensers();
  ...
  // Filtrar surtidores por búsqueda y empresa
  const filteredDispensers = useMemo(() => {
    ...
    return filtered;
  }, [dispensers, searchTerm, idCompany, user?.role]);
  ...
  const handleEdit = (dispenser: Resource) => {
    setEditingDispenser(dispenser);
    setFormData({
      idType: dispenser.idType,
      idCompany: dispenser.idCompany,
      idBusinessUnit: dispenser.idBusinessUnit,
      nativeLiters: dispenser.nativeLiters,
      name: dispenser.name,
  ...
  const handleDeleteClick = (dispenser: Resource) => {
  ...
  const handleDelete = async () => {
    if (!deleteDispenser) return;

    try {
      await deactivateMutation.mutateAsync(deleteDispenser.id);
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Dispensers");
    XLSX.writeFile(
      wb,
      `dispensers_${new Date().toISOString().split("T")[0]}.xlsx`
    );
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button
            ...
            disabled={filteredDispensers.length === 0}
            sx={{
              borderColor: "#10b981",
              color: "#10b981",
              fontWeight: 600,
      ...
      <Grid container spacing={3}>
        {filteredDispensers.map((dispenser) => {
          const company = companies.find((c) => c.id === dispenser.idCompany);
          const businessUnit = businessUnits.find(
            (bu) => bu.id === dispenser.idBusinessUnit
          );
          return (
            // @ts-expect-error - MUI v7 Grid type incompatibility
            <Grid xs={12} sm={6} md={4} lg={3} key={dispenser.id}>
              <Card
                elevation={0}
                sx={{
                  background: "white",
                  ...
                  opacity: dispenser.isActive !== false ? 1 : 0.7,
                  "&:hover": {
              ...
              >
                ...
                >
                  ...
                  {/* Capacidad */}
                  {dispenser.nativeLiters && (
                    <Box sx={{ mb: 1.5 }}>
                      <Typography variant="body2" color="text.secondary">
                        Capacidad: {dispenser.nativeLiters} L
                      </Typography>
                    </Box>
                  )}
                  ...
                  <Box sx={{ display: "flex", gap: 1, mt: "auto", pt: 1 }}>
                    ...
                    <IconButton
                      size="small"
                      onClick={() => handleDeleteClick(dispenser)}
      ...
      {/* Empty state */}
      {filteredDispensers.length === 0 && !isLoading && (
      ...
      >
        <DialogTitle>
          {editingDispenser ? "Editar Surtidor" : "Nuevo Surtidor"}
        </DialogTitle>
        <DialogContent>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ¿Estás seguro de desactivar el surtidor{" "}
            <strong>{deleteDispenser?.name}</strong>?
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
